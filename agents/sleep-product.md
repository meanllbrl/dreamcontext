...
|  | `_dream_context/automations/**` (manifest/cache/output — READ output files for material via `pendingOutputsSince`'s file list, NEVER edit or delete them; not yours to own, see "Automation output consumption" below) |
...
13. **Never edit, move, or delete anything under `_dream_context/automations/`.** You read output files for material; the automations subsystem owns them entirely, including their cache and manifests.
