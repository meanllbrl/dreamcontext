...
<p align="center">
  <img src="public/image/diagram-federation.png" alt="Two halves of cross-vault federation. PUSH (sleep-driven, automatic): Vault A's sleep cycle runs computeDigest over its FULL corpus (knowledge+feature+task+changelog+memory) minus federated and already-synced docs, BM25-ranked to the peer's interest, through a consent gate into Vault B's federation-inbox; B's federation drain ingests each entry as a first-class knowledge/<slug>--from-A.md (federated:true + provenance), where normal recall then surfaces it like any local doc. PULL (on-demand): recall --connected runs crossVaultRecall across A plus consenting peers live, excluding federated docs, returning merged hits tagged by vault. The only knobs are direction, status, consent, and topics — there is no when-X-read-Y trigger." width="900" />
</p>
...
The desktop launcher exposes both halves visually. Its **federation network view** draws every vault as a card and lets you wire two relationship kinds by clicking source → target: a violet **reads** wire (the pull half — `out`/`both` + the target's Readable gate) and a teal **sync** wire (the push half — it writes `out` on the source *and* `in` on the listener in one gesture, satisfying the consent rule the CLI enforces). A per-node panel and an always-on connections list render the stored directions back as plain sentences ("X reads Y live", "Y listens to X's changes"), each removable in one click — a direct, auditable window onto the `.connections.json` direction model rather than a new mechanism.
