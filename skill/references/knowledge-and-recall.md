...
- **Write them AWAKE, in-session** — when an approach worked twice, or the user teaches one, offer-and-confirm and write `knowledge/patterns/<slug>.md` (normal knowledge frontmatter; good `description` + tags so recall surfaces it), then run `dreamcontext patterns sync` so it gets its `/` entry. Never defer to sleep. **Name it after what a user would SAY, not after the code** — the filename and H1 become the pattern's triggers, so `plan-ozeti` fires on "özetle" while `comm-fmt-v2` fires on nothing. Confirm with `dreamcontext patterns match "<the phrase you expect>"`.
...
- **A matching pattern is already IN your context — it is not something you fetch.** The `UserPromptSubmit` hook runs a deterministic gate over `knowledge/patterns/` on every prompt and INJECTS the full prose of every pattern the message names, under `— Project patterns (N triggered) —`. Anything past the character budget is listed as a path you must read. Follow what arrives: patterns are decided positions, not suggestions; if one is wrong for the case, say which part and why before departing from it. Precision is tuned so the gate stays silent on prompts that name nothing — evidence required scales with prompt length, so a long dump containing one incidental keyword fires nothing.
- **KEEPING A PATTERN TRUE IS PART OF THE TASK, not a sleep chore.** When the user contradicts, narrows, or extends an injected pattern — or you and the user settle on something it does not say — **update that pattern file before the task is finished**, and say in your reply which pattern changed and how. The prompt hook repeats this on every injection because it is must-not-miss (see `knowledge/patterns/hook-delivered-must-not-miss-rules.md`). Why it cannot wait for sleep: a stale pattern is worse than no pattern, because it keeps being injected and keeps being obeyed, so the user pays the same correction twice; and by the time a cycle runs, the ARGUMENT that produced the correction is gone and only its conclusion survives. How: edit the section that is now wrong — do not append a contradicting paragraph underneath the old one — keep the file inside the ~150-line discipline, and add a dated line to its changelog if it has one. When to refuse: a one-off preference for this task is not a rule; say out loud that you are treating it as one-off and leave the pattern alone, rather than silently skipping.
- **Triggers are DERIVED, never authored.** A pattern's own filename and title are its triggers, folded through an ascii-first, Turkish-aware matcher, so a vault whose owner never heard of this still gets its patterns fired (`özetle` reaches `plan-ozeti-…`). `triggers:` / `aliases:` in the frontmatter are an OPTIONAL addition for a word the name does not contain — never a prerequisite. Inspect any of it with `dreamcontext patterns list` and `dreamcontext patterns match "<prompt>"`.
- **Browse/load surfaces**: `dreamcontext patterns list`, the auto-installed `patterns` core skill (`/patterns` lists, `/patterns <slug>` loads; canonical source `skill-patterns/SKILL.md`), and one generated `/` entry per pattern (`.claude/commands/pattern-<name>.md`). The entries are **self-maintaining per project**: the SessionStart hook regenerates them whenever the pattern set changed (fingerprint-gated, so the steady state writes nothing), and setup/update/sleep do the same. `dreamcontext patterns sync` exists to force it, not because anyone must remember to. The shims hold a path, never pattern prose, so they cannot drift; they are commands rather than skills so a 40-pattern vault costs nothing per session. A project without the folder degrades to "no patterns yet".
