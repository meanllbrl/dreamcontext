...
3. **Build the brief inline** (cheap CLI, no transcript content):
   ...
   - **Sessions go in the brief OLDEST → NEWEST, with their `stopped_at` and `task_slugs`** — `sleep-tasks` resolves contradictions by *latest session wins*, and it can only do that if the order it is handed is the order they happened in:
     ```bash
     jq -r '.sessions | sort_by(.stopped_at) | .[] | "\(.stopped_at)  \(.session_id)  [\(.task_slugs | join(","))]"' _dream_context/state/.sleep.json
     ```
     Paste that list into the brief and add the line **"later sessions override earlier ones"**. Without it a specialist reading sessions in file order can file a task for work a later session cancelled — the exact failure this ordering exists to prevent.
...
10. **`dreamcontext sleep done "<summary>" [--flag <key>::<label>[::<task-slug>]]...`** — clears pre-epoch state, resets debt, writes a history entry. Before running it, collect every recidivism flag the specialists reported (sleep-state's Recidivism flags block, sleep-tasks' grooming (e)) and pass each as its own `--flag` — repeatable, ONE `--flag` per flag, never comma-separated (e.g. `--flag recurring-task:fix-x::"fix-x recurred"::fix-x --flag ceiling-blocked:2.memory.md::"blocked again"`). At 3 consecutive cycles on the same `key`, `sleep done` itself surfaces an escalation ask and bumps the linked task's priority — you relay what specialists reported, you don't compute the streak. **The `task-candidate:<key>` family is the one exception to that escalation.** `sleep-tasks` emits one for work it judged real but only INDIRECTLY evidenced ("it was discussed") — deliberately not filed, so there is no task behind it: relay it like any other flag (`--flag task-candidate:<slugified-topic>::"<the evidence, one line>"`, no task slug), and `sleep done` keeps its consecutive streak but EXCLUDES it from escalation — no ask, no priority bump, because there is no task to bump — printing one line instead: `Deferred task candidates: n (m seen ≥2 cycles — sleep-tasks files them when re-observed).` A candidate a later cycle independently re-observes is filed then; one that is not re-emitted is dropped by the same streak contract as every other flag. Never invent or drop these keys yourself — they must stay byte-stable across cycles or the streak restarts. **Thesis promotion note:** when `sleep-learn` reports a thesis that flipped `validated`/`invalidated`, relay its promotion proposal to the user in your summary (it is a decision ask, not an action taken) — do not run `theses promote` yourself; that command only records a promotion that already happened in a later session. A chronic-open thesis (`sleep-learn`'s ≥3-cycle report) becomes its own `--flag chronic-open-thesis:<slug>::"<label>"::<slug>` alongside the other recidivism flags. (If a remote backend — ClickUp or GitHub — is active and any task pushes failed, this auto-retries once, then errors loudly with the failed slugs.) **Brain sync also fires here:** if a shared brain repo is active (`brainRepo.autoSync`), `sleep done` runs a foreground `brain sync` (fetch→merge→commit→push). Sync failure never fails sleep — but on a prose conflict it pauses with `awaiting-agent` and prints the `/dream-sync` prompt; run that skill, then `brain sync --continue`. Full model → [brain-sync.md](brain-sync.md).
11. **Report** the consolidated summary to the user — include the semantic dedup digest line `sleep done` printed (`Semantic dedup since epoch: X merge / Y review / Z create (N decisions).`, when non-zero) and any recidivism escalation asks, the `Deferred task candidates: …` line, or curator-task creation it surfaced.
...
| `sleep-product` | Knowledge files + feature PRDs + patterns | Creates/reconciles `knowledge/*.md` and `knowledge/features/*.md` (typed knowledge, `type: feature`), processes staleness flags, maintains the knowledge index + taxonomy. **Pattern lifecycle** (`knowledge/patterns/*.md`) — but NOT their content: creates a pattern when `sleep-state`'s recurring-practice signals warrant one (two occurrences = a pattern), condenses one that has outgrown the ~150-line discipline, retires stale ones (archive-before-delete), and runs `dreamcontext patterns sync` after any create/rename/retire so the generated `/pattern-*` entries match the vault. **Sleep does NOT fold user corrections into patterns.** That is the awake agent's job, in the task where the correction was made (the prompt hook says so on every injected pattern): by the time a cycle runs, the argument that produced the correction is gone and only its conclusion survives, so a sleep-written rule is a rule nobody can defend later. If a cycle's transcript shows a pattern that was contradicted and never updated, REPORT it as a missed awake rule — name the pattern and the correction — rather than quietly rewriting it. Never touches `knowledge/archive/` — that's `sleep-state`'s escalation output. **Also reads new automation output** (never edits or owns it) and folds anything genuinely new into the right knowledge file — see "Automations" below. |
...
2. **Sleep never OWNS an automation's files.** No specialist creates, edits, or retires `_dream_context/automations/*.md`, its cache, **its human-in-the-loop questions (`automations/hitl/<slug>/*.json`)**, the machine-local registry, or the dispatcher plist. Nobody owns the questions store either — like the run lock and the sidecar it sits beside, it is the automations subsystem's own machine-local state, written only by the runner and answered only by a human (chat, CLI, or the per-automation Telegram bot), never reconciled by a specialist. Same boundary again one level out: a per-automation Telegram credential (`~/.dreamcontext/telegram/<slug>.json`) and a session binding (`~/.dreamcontext/automations/<slug>.sessions.json`) live outside the brain entirely, at the HOME boundary, and sleep never reads or touches either. An automation manifest is authored directly by a user or an agent in-session (`automations create`, then a live-tested `run --force`), and its cache and output files are written deterministically by the runner and the dispatcher tick, never by an LLM reconciling "current truth" the way `sleep-tasks` reconciles a task body. This mirrors Lab insights, which are likewise not owned by any sleep specialist.
