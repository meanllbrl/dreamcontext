...
**One chat, one scroll.** Each pane's stick-to-bottom state belongs to its OWN session: the auto-scroll keys on that session's conversation-model identity (not "every render"), so activity in one chat can never move a split neighbour's transcript. Unsticking takes an actual UPWARD scroll — content growing under a pinned view can't be mistaken for the user leaving, which is what used to kill auto-scroll mid-answer — and a `ResizeObserver` re-sticks after height changes React never rendered (an image finishing its load, the composer growing a line, a split/resize, the pane being re-homed into another slot, which resets `scrollTop` to 0). Sending re-arms sticking; while unstuck, a *↓ Latest* pill floats over the transcript's bottom edge.
