...
The sidebar is grouped into **Workspace** (Tasks, Roadmap, Hypotheses, Insights, Automations, Council), **Memory** (Core, Knowledge, Features, Taxonomy), **Brain** (Map, Sleep Cycle), and **Control** (Packs, Settings), plus a "What is this?" page.

**What's in it:**
- **Search + Ask** (top bar) — the dashboard's recall surface: a scoped recall widget that runs the same engine as the CLI (empty query = browse; typing = live debounced BM25; "Intelligent" = a Haiku intent pass), plus an in-app **interactive Claude Code** agent (multi-session tabs, split panes; desktop-gated, read-only by default) with an optional **Chat view (BETA)** — see below. The overlay's chrome is a single 44px row (window controls + tab strip merged, no page title), and tab kinds read as mono glyphs rather than emoji: `◇` Claude agent, `◆` Claude chat, `>_` plain login shell.
...
- **A file that can't be shown says why** (`chat/FileUnavailable.tsx`): a media element reports failure and nothing else, so every surface that draws one — the image lightbox (`ImageViewer`'s `fallback` slot), the slide-over's media preview — hands the failure to one shared card that asks the endpoint with a one-byte ranged GET and reports what it hears: **outside the project** → *Allow access* (granting the path the SERVER resolved, then re-requesting under a bumped `reload=`), **404** → it's gone, **bytes arrive but the element refused them** → this window can't decode it. Before 09-06 the lightbox drew the engine's broken-file glyph on a full-window black screen for the single most common case — a screenshot in `/tmp` — and the panel pointed at an inline consent card that only exists when the answer rendered the file inline. Esc in any of these viewers also yields to them from the surface's own collapse handler (presence test on `.image-viewer, .pdf-viewer, .fullscreen-overlay`), so closing a picture no longer closes the chat behind it. End-to-end: `npm run verify:file-preview`.
...
- **Rich output — the agent KNOWS it is in Chat.** A chat-spawned `claude` is byte-identical to a terminal-spawned one, so by default it writes for a TTY: it finishes a board, says "done", and names the file. Every chat spawn therefore appends a short surface briefing (`src/server/chat-surface.ts`, handed over as `--append-system-prompt-file` so the login-shell argv stays free of prose; a failed write degrades to the un-briefed agent, never to a failed spawn) telling the agent what its markdown becomes here. Five shapes, all ordinary markdown so a terminal or a raw transcript still reads fine:
  ...
  - **a fenced ```dream-html block** — HTML the agent WROTE, rendered inline. **Prose is the default** (owner, 2026-09-11); a block earns its place only as a drawn diagram, an interactive view or a deck — see the dedicated bullet below.
...
- **`dream-html` — the agent writes the presentation (2026-08-26; replaced the typed `chart` + `page` payloads), and since 2026-09-11 does so RARELY.** The briefing's section is "Prose first. Draw only what prose cannot hold": markdown is the default, and exactly three things earn a block — a **diagram** (the shape is the message: it branches, merges or loops), an **interactive view** (the reader filters, toggles, hovers data), a **deck** (several screens for the fullscreen button). Named as never-a-block: a short answer, a status, findings, a 2-4 step chain, two options in two sentences, one key/value, cards holding paragraphs. The operational test is "delete the block and keep the prose — if nothing is lost, it was ceremony". Seven owner screenshots of blocks harder to read than the prose they replaced drove this; `tests/unit/chat-surface-lockstep.test.ts` pins the inversion so the draw-by-default sentence cannot come back. A fenced ```dream-html``` block is rendered in a **network-less sandboxed iframe** wearing dreamcontext's own class kit. The trade is deliberate: the old contract let the agent express only the five renders and seven widgets we had pre-built, so a flow, a tabbed comparison, a timeline or two candidate designs side by side either fell back to prose or cost a new widget — the owner reported `page` had never once been used. We stopped constraining WHAT can be said and constrained what the markup can DO instead.
  ...
  - **The `dc-` class kit** (`chat/chat-html-kit.css`, mirrored as a drift-tested TS string because vitest stubs `.css?raw` to empty) is a READING and PRESENTATION surface, not the Lab's 232px card: headings and prose, cards, stat tiles, comparison tables, callouts, bars, funnels, **steps / timeline / compare-options / tabs**, a slide mode — and **a diagram engine**: `dc-graph` of `dc-node`s and `dc-edge`s (`data-from`/`data-to`/`data-label`) is DATA until `chat/chat-html-graph.js` (mirrored as `KIT_GRAPH`, riding in the srcdoc head and in the export) ranks the nodes by their edges, breaks cycles (drawn dashed), routes long edges between nodes, orders each rank to cut crossings, measures every node, places them and draws each edge as an SVG path with an arrowhead and its label. A chain that fits runs left to right, anything else top down; it re-lays on width change, so nothing ever wraps (the old chip-row flow wrapped, then became a "↓" stack — neither was a diagram). Tone is a border colour, never a fill. **And since 2026-09-13 it is a VIEW, not a picture** (owner screenshot: "bu görünüm interaktif değil yaklaştırılamıyor merkezde değil … tıklanamıyor iç içe yazılar var"): the drawing sits on a stage that is CENTRED in the pane (a 270px ladder used to hug the left edge of a 1050px pane), drags to pan, zooms on ctrl/⌘-wheel — which is also how a trackpad pinch arrives — or the corner control that rests invisible so exports stay clean, and answers `+` / `-` / `0` / arrows / Esc from the keyboard. A **plain wheel is deliberately untouched**: the block lives in a transcript, and a diagram that ate the page's scroll would be worse than one that could not be zoomed. Hovering a node lights it, its edges and their far ends; clicking HOLDS that trace and dims everything else, so one path through a dense ladder reads on its own — every node is tabbable and answers Enter/Space, and the pointer is captured only after a 4px move so a pan never eats a click. Labels are PLACED, not pinned: candidate boxes slide along the finished path and step off it, scored against every node and every label already placed (the old midpoint pin put "+100k sonra" on two nodes, because the reserved waypoint and the drawn midpoint disagreed on a back edge with an even number of points). `dc-flow` is retired from the briefing but still rendered (as a chain graph) so old transcripts keep working. Geometry is proven in real Chromium by `npm run verify:chat-html` (`runGraph`: anchored arrows, no overlaps, labels off nodes, mutation-checked against a build without the engine). The agent writes our classes and the block looks native in both themes; the briefing's class list is pinned as a subset of the kit by `tests/unit/agent-board-assets.test.ts`, so a class named to the agent but undefined in CSS fails the suite instead of silently rendering unstyled.
...
- **`dream-view` — the eight things HTML must NOT be.** A fenced ```dream-view``` block — one JSON object, `"type":"insight"|"checklist"|"secret"|"run"|"pin"|"progress"|"checkout"|"agent-thread"` — is what survived the retirement of `chart`/`page`, plus the two added on 2026-09-19 for the things a sandboxed block can never be: a PRIVILEGED submit and a live process. Schema, caps and the pure validator live in `dashboard/src/lib/chatViewSpec.ts` (`parseViewBlock`); the fence is found and stripped by `chatActions.ts`, which parses `dream-view` and `dream-html` in **one pass** so blocks render in WRITTEN order rather than all views above all HTML. Rendering is `chat/ChatViews.tsx`. Max 4 view blocks per message. Pinned in lockstep with `src/server/chat-surface.ts`'s briefing by `tests/unit/chat-surface-lockstep.test.ts`. A `{"type":"chart"}` or `{"type":"page"}` block from an old transcript degrades down the ordinary unknown-type path — a notice naming the type, prose untouched.
  ...
  - **`secret`** — `{"type":"secret","id","title","file?","fields":[{"key","label?","hint?"}],"intro?","submitLabel?"}`, a masked field drawn IN the transcript whose value the agent never sees. On Submit the browser POSTs it to `POST /api/agent/secret` and the **server** writes it into a `.env`-family file; what goes back into the conversation is a RECEIPT built server-side (key, file, char count, `sha256:` prefix, added/updated) and posted verbatim by the client — composing that sentence in the browser would put the surface that holds the value one typo away from interpolating it. **This is the opposite promise to a checklist's `wants:"secret"`, deliberately**: that field submits as markdown into the transcript and says so on its face, which is right for a value the agent must USE and wrong for a credential whose destination is a file. Both notes live next to their own field; unifying them would make one of them a lie. **Every guard is in `src/lib/env-secrets.ts` and re-run on the raw body** (the client's copies only shape the card, and are pinned to the owner by `tests/unit/chat-secret-mirror.test.ts`): the path must be `.env`/`.env.<suffix>`, optionally up to 3 directories deep, inside the project; the parent is **realpath-contained** and the leaf **lstat-refused if it is a symlink** (a planted `.env → ~/.ssh/authorized_keys` would make this route an arbitrary-file overwrite, which is the read-side ★★★ rule applied to a write); a file **git already tracks is refused outright** (a secret there is one `git commit -a` from being published, and nothing can un-publish it); the `.gitignore` entry is placed **before** the write and a failure to place it aborts the write — the ordering guarantee `task-backend/secrets.ts` has carried since issue #11 — asked of git via `check-ignore` so a parent `.gitignore` or a `.env*` pattern counts instead of a redundant appended line. The file is written 0600 (and an existing 0644 `.env` is tightened on write); the value's ends are trimmed but never its middle (a PEM's newlines are load-bearing), and the key's LAST assignment is the one rewritten, because that is the one dotenv ends up using — rewriting the first would leave the user looking at their new value while the app read the old one. **NOT desktop-gated**, unlike the PTY routes: it is a bounded write into one file in the project the request already named, behind the same `isCrossSiteWrite` + remote-access token gates every other write has, and gating it out would break the case it was asked for — pasting a key from the phone. Caps: 8 keys/card, 8192 chars/value. No value ever reaches a log line, an error message or a response body.
  - **`run`** — `{"type":"run","id","command","why?","cwd?"}`, a command with a ▶ that opens a **real PTY inside the transcript**. For what the agent genuinely cannot do headless: a browser login, an OTP, a sudo password, any `y/n` — those either hang with nobody at the keyboard or send the user to another terminal window, which ends the session. Transport is the existing bridge with a new kind: `/api/agent/terminal?kind=exec&cmd=…&cwd=…` spawns `$SHELL -ilc 'exec <command>'` so the PTY's exit IS the command's exit, and pushes `{type:'exit',code}` as a **binary** control frame (output is always text frames) before closing. `kind=exec` adds **no privilege over `kind=shell`**, which has handed out a bare interactive login shell in the vault root since 0.22 behind the same desktop+loopback gate — it narrows one. The command is **not vetted and deliberately so**: it is shown in full, nothing runs until the user presses ▶, and an allowlist that blocked `rm` while allowing `sh -c` would add no safety and would imply a check that had not happened. What IS enforced is that the bridge runs **the command the card showed**: a newline truncates rather than collapsing to a space (welding two statements into a third, different one), control bytes are stripped, and the client refuses a multi-line command outright. `cwd` is relative, `..`-free, and realpath-contained. On exit the card posts the outcome back into the conversation through the composer's own ⏎ chain (`postToSession` — steer the running turn, queue only when that cannot land), so **the agent continues by itself**; the output tail is ANSI-stripped and capped at 40 lines / 6000 chars. **The output switch is the honest part**: sharing the tail is the default because that is what makes the next line useful, but a command that PRINTS a credential (`firebase login:ci`, `gh auth token`) would put it in the transcript — so it is a checkbox the user flips *before* pressing ▶, never a judgement the agent makes for them. The terminal is `chat/InlineTerminal.tsx`, built on the SAME `termCore.ts` factory the agent terminal uses (theme, cell metrics, font-ready open), not a second hand-rolled xterm; unmounting the card closes the socket, which kills the PTY rather than orphaning it.
  - **The same ▶ on a permission card.** A Bash permission request now offers **Deny / ▶ Run here / Allow**. "Run here" opens that same inline PTY on the exact command, and when it exits the permission is answered `deny` **carrying the run's report as the tool result**. Deny, not allow, is load-bearing: allowing would run the command a second time, headless, straight back into the prompt it just hung on. The message says plainly that the user ran it, so "denied" never reads to the agent as "refused".
  ...
  - **`checkout`** — `{"type":"checkout","path":"/abs/path"}` or `{"type":"checkout","reset":true}`, and it is the odd one out: **nothing is drawn for it.** It is the channel an agent corrects the shelf's branch chip on when its work lives in a checkout it is not standing in — the split-brain case, cwd in the brain repo while the edits are in a linked repo's worktree, which produces no `EnterWorktree` frame and no transcript `cwd` to observe. **Applied by the SERVER**, off the same stdout stream it already reads tool frames on (`src/server/checkout-directive.ts`), never by the client: a directive applied in the pane would be a fact that exists only while someone is watching. The path is validated by the same governed-repository gate a tool frame's is (`session-cwd.ts`) — a directory outside this project and its linked repos is refused — and **the outcome is stated either way**, on the same banner the fresh-start branch guard uses, because a silent refusal leaves an agent repeating a claim nobody applied. A claim **outranks the transcript** (it is the only source that can describe work happening where the session is not standing); `reset` withdraws it and hands the answer straight back to the observers, it does not pin the project root. The client parses the block only to keep it off the unknown-type notice path and to refetch the facts at once instead of at the next 15s poll.
  - **`agent-thread`** — `{"type":"agent-thread","slug":"<automation-slug>","run":"<ISO fireAt>","limit":N}`, a compact card for one agent's channel: photo, title, the last few entries, an unread pill, Reply, and Open session through the same run-resume bridge the Agents page uses. **Derived from disk, exactly like `insight`** — the agent names its channel and `chat/AgentThreadCard.tsx` reads the thread, so any `entries`/`text`/`messages` key the agent asserts is **dropped with a notice**: a thread is a record of what happened, and an agent writing its own entries into the card would be composing a transcript rather than reporting one. An unsafe slug is rejected; `limit` is clamped. Reply here is a one-line control, not a mounted `Composer` — the card is a ~200px summary inside a transcript that already has a composer, and the full one lives in the Agents page's thread panel where a reply conversation actually happens.
...
- **The pinned shelf — where `pin` and `progress` live.** Two kinds of information do not belong in a transcript: session facts the user needs for the WHOLE session, and how far along a long run is. Both get a shelf **docked to the composer's top edge**, sharing its width and corners, outside the transcript's scroller — so after scrolling to the top of a long conversation and back it is still there, unchanged. It **does not react to scroll** at all (no hide-on-scroll-up, no reveal-on-scroll-down, no resize): scrolling up is usually the user hunting for a fact the shelf already holds, so hiding it then is backwards, and scroll-reactive chrome re-opens a bug class this surface already paid to close twice.
  - **Resting state is ONE tag line**, and it is never empty ceremony — the branch and, in a linked worktree, a `worktree` marker come from the server (`GET /api/agent/session-facts?session=<uuid>`: `git symbolic-ref --short HEAD`, worktree detected by `--git-common-dir` ≠ `--absolute-git-dir`), so they are true without the agent asserting anything and carry no dismiss button. The facts are read at the checkout **that session** is working in, resolved in one fixed order — the agent's `checkout` claim, then the conversation transcript's own `cwd`, then `EnterWorktree` frames, then the project root — and the accepted set of checkouts is every repo the vault **governs**: its own, plus each `dreamcontext link`ed code repo present on this machine. That last clause is not a detail: with a brain split from its code, every worktree the agent actually works in belongs to the linked repo, and gating on the vault's own repository alone reported the brain's `main` for all of them (owner screenshot, 2026-08-28). `worktreeAllowed` alone is read at the VAULT rather than the checkout, since it is a property of the brain. **And when the agent declares nothing at all**, the chip still says so: the server counts WRITE frames (`Edit`/`Write`/`MultiEdit`/`NotebookEdit`, read off the same stream — never `Read`/`Grep`, and never `Bash`, whose paths live inside a shell string) per checkout, and `elsewhere` marks the chip when they landed in a governed checkout that is not the one it names (`src/lib/session-edits.ts`). **It warns and never follows** — a file path is a side effect, not a statement, and a chip that renamed itself off an inferred signal would leave no way to tell a measured checkout from a guessed one. Writes under `_dream_context/` are excluded, and that exclusion is load-bearing rather than tidy: a session working in a worktree logs tasks and changelog entries into the brain's own checkout by design (19 of 19 write frames across the live sessions measured on 2026-08-28 were brain writes), so counting them would leave this warning on during every correct session. The count is composed per request, OUTSIDE the 12s git memo, because it moves while a turn runs. The **dev-server port is the agent's to pin** — nothing on this machine can attribute a listening port to a session, and the agent ran the command.
...
- **`/mcp` from Chat — the MCP panel.** The headless engine does not refuse `/mcp`, it ANSWERS it: a result frame carrying `local_command: "mcp"` and *"N MCP server(s): … Use `/mcp` in the terminal for details."* (CLI 2.1.276) — a turn spent to be sent to another app, while unauthenticated servers stay dead with nothing in the window able to say so. So the composer intercepts `/mcp` (`isMcpCommand`, leading form only, so "what does /mcp do?" still reaches the model) and opens a **slide-over panel** listing every server THIS CONVERSATION has, with **Sign in** / **Sign out** where an action can actually work.
  ...
  - **Every leg runs in the PROJECT**, because a server can be project-scoped (`.mcp.json`) and that scope resolves from the working directory. The route is deliberately NOT vault-agnostic.
  - **A button is only drawn where it can work.** `source` says where a server came from; a `dynamic` one (handed to a sandboxed session by `--mcp-config`) is invisible to `claude mcp login` — "No MCP server named …", measured — so those rows get an explanation instead of a sign-in, pointing at Settings → MCP servers.
  - **The verdict after a sign-in is a RE-READ of the session, never the exit code** — an OAuth abandoned in the browser exits 0 — and the login child's output is discarded at the spawn, because an interactive OAuth's stdout can carry a callback URL bearing an authorization code. No token is displayed, stored, logged or pasted. An unrecognised status is shown verbatim as *unknown* rather than rounded to a known one.
- **Settings → MCP servers — the scope a TEAM can share.** The answer to "my colleague uses dreamcontext and can't reach the MCP servers": the repo's **`.mcp.json`**. Measured — a Claude account that had never opened the repo reported a server there `connected` on its first run, with no approval step, because a headless `-p` session resolves project scope from the working directory. Nothing is copied, no credential is duplicated, and a teammate gets the servers by cloning. The Settings section lists what the repo shares, lists this machine's own servers as candidates, and moves one in on a click (`POST /api/agent/mcp/project/adopt`). **It refuses to publish a secret**: `.mcp.json` is committed and a brain repo is synced across a team, so every `env`/`headers` value that is not already a `${VAR}` reference is rewritten into one and the required variable names are reported; a server whose URL carries its own token is refused outright, because splitting that URL changes what the server is. Values never reach the client either — key names and masked targets only. Each person still signs in once, from the chat panel, to any server that uses OAuth: a credential belongs to one person.
- **Installed, but invisible to the shell.** The third prerequisite state, and the one that used to read as "not installed at all": `npm install -g @anthropic-ai/claude-code` (like the CLI's own `install.sh`) puts the binary in `~/.local/bin`, which is on no default PATH — so an install that skipped the closing `export PATH="$HOME/.local/bin:$PATH"` echo leaves `claude` on disk while every `$SHELL -ilc 'exec claude …'` spawn 127s. Two halves fix it (`src/lib/claude-path.ts`): every spawn we own — terminal PTY, chat, tab titling, hook recall — gets `claudeAwarePath()`, which APPENDS the discovered install directory (never prepends, so a `claude` the user's rc already resolves is never redirected), and the installer performs the missing echo itself, idempotently under a `# dreamcontext:` marker, into the rc that shell actually reads (zsh → `~/.zshrc`, bash → `~/.bashrc` plus `~/.bash_profile` only if it exists, fish → `config.fish` with `set -gx`, else `~/.profile`). `GET /api/agent/capabilities` reports it as `claudePathBroken`, so **Settings → System** shows an amber *"Installed, not on your PATH"* with a one-click **Fix PATH** (`POST /api/agent/install {target:'claude-path'}`) instead of a false "Missing" — the in-app surfaces already work, but the user's own terminal can't run `claude` until the rc line lands.
...
**Question card (AskUserQuestion, 2026-09-26).** The chat spawn sets `CLAUDE_CODE_QUESTION_EXTENDED=1` and `CLAUDE_CODE_QUESTION_PREVIEW_FORMAT=html` (`CHAT_QUESTION_ENV` in `routes/agent-chat.ts`), which switch on the CLI's extended question schema: a call-level `title`, a per-question `description`, `text`/`number` questions, and an HTML `preview` per option. The card leads with the title as a **context strip** and the description under the question, because the owner reaches a card from a notification without having read the turn — the surface briefing's "Asking — a card read cold" section tells the agent to always fill both. Surfaces by shape: option **rows** (1–4 keys pick); an **A/B/C board** of tiles when any option has a `preview` (the fragment drawn in the `dream-html` sandbox with project `<img>`s inlined as data URLs, or — when the preview is nothing but one `<img>`/`<video>` with a project path — drawn natively, so a clip plays in place); a **swipe deck** when `metadata.source` is `"swipe"` and every question is a 2-option single choice (drag right = first option, left = second, ←/→ too); a slider for `number`, a text box for `text`. One **always-visible free-text field** sits under every choice: with an option picked it is a note (sent as `annotations[question].notes`, which the model reads as `notes: …`); with nothing picked, or with the **Other** row picked, it is the answer (multiSelect: one more value after the ticked ones). Other is a plain row again (key = option count + 1) — picking it only re-labels and focuses the field that is already there, so nothing expands on click and the card no longer shoves itself up the bottom-following transcript. Every preview has a **fullscreen door**: HTML through `HtmlView`, a lone picture through the shared `Lightbox` (zoom to the pixel), a lone clip through a full-window player. Those overlays are portaled and React bubbles portal events through the tree, so the card's vote/drag/key handlers ignore any event that did not start in their own DOM. **Unclear? Ask again** answers with a deny whose message tells the agent how to re-ask (the CLI's input schema has no free-response field). The request is echoed back verbatim on answer, so fields the parser does not model survive. Pinned by `survey-answers.test.ts`, `chat-protocol.test.ts`, `chat-question-env.test.ts` and `npm run verify:chat-questions`.

**Known limitations (beta, tracked but not blocking):** the composer's **📋 Task** button is a no-op placeholder today (the task-picker UI itself isn't built — attaching a task chip to a draft is not yet possible from Chat view); the **survey card's "answered" receipt is transient** — once `ChatSession` removes a resolved question from its pending list, re-scrolling to that point in the conversation does not show a persisted "you chose: …" row (a true persistent receipt needs a data-model change beyond this pass); a `dream-view` can only render **after** the prose, never mid-paragraph — splitting `AssistantMessage`'s single `MarkdownPreview` mount to interleave one is a bigger rewrite than this pass took on, so use a page's own `title`/section headers instead; the checklist window's `alwaysOnTop` is plain JS and does **not** float over another app running in macOS full-screen Space (a normal window, yes) — a Rust `NSPanel` treatment was deliberately deferred; a note typed into an **ordinary** (non-`secret`) checklist item is still written to `localStorage` for up to 7 days — there is no way to tell a pasted key apart from a note by validation, so this is mitigated with a UI hint, not fixed; and a page/card image's query string and fragment are stripped but its path is not — an attacker-chosen host can still path-encode data and observe IP/timing from a near-zero-click image load, a residual risk that the query-strip + `no-referrer` + `lazy` mitigations reduce but don't eliminate.
