...
- **`/mcp` from Chat — the MCP panel.** The headless engine does not refuse `/mcp`, it ANSWERS it: a result frame carrying `local_command: "mcp"` and the text *"N MCP server(s): … Use `/mcp` in the terminal for details."* (CLI 2.1.276) — a turn spent to be sent to another app, while unauthenticated servers stay dead with nothing in the window able to say so. So the composer intercepts `/mcp` (`isMcpCommand`, leading form only, so "what does /mcp do?" still reaches the model) and opens a **slide-over panel** listing every configured server with its health, plus **Sign in** / **Sign out** per row. Everything shown comes from the CLI itself — `GET /api/agent/mcp` runs `claude mcp list` and parses it (`src/lib/claude-mcp.ts`; there is no `--json` on this command), and the two actions run `claude mcp login|logout <name>` for ONE server. Three rules carry the design: (1) **the panel mirrors the SPAWN, not a config directory** — a chat session runs under its account's config dir AND, when that account is a sandbox, is handed the machine's own servers by reference (`--mcp-config <shared file>`, since no MCP key is ever copied into a sandbox). Asking the config dir alone lost every local server the user had (owner report 2026-09-18: measured 24 listed against 32 actually available), so `listMcpForSession` runs BOTH listings in parallel — the account's, plus the shared file's servers listed against the real home where they are configured — and merges them, each row carrying the `origin` it came from. That origin also decides where a sign-in lands: a shared server's credential belongs in the real home, and a client CLAIMING `shared` for a name the shared file does not carry is refused rather than trusted; (2) **the verdict is a re-probe, never the exit code** — a login abandoned in the browser exits 0, so the route re-runs `claude mcp get <name>` and reports what that says, and a row therefore never claims a tool the agent does not have; (3) **the login child's output is discarded at the spawn**, because an interactive OAuth's stdout can carry a callback URL bearing an authorization code — the same rule `claude auth login` follows. No token is ever displayed, stored, logged or pasted. A status the CLI has not printed before is shown verbatim as *unknown* rather than rounded to the nearest known state. Verified end to end in the real app: `npm run verify:chat-mcp` (36 assertions, incl. a leak canary printed by the login child that must appear in no response and no pixel of the DOM, and a sandboxed account that must still see its local servers).
