...
### `doctor --json` — the diagnostic contract
...
| Command | Description |
|---|---|
| `tasks list` | List/filter/group tasks (excludes terminal — done- and cancelled-kind — tasks by default). Flags: `-s/--status`, `-a/--all`, `--tag <t>` (repeatable, AND), `--any-tag <t>` (repeatable, OR), `--version <id>` (sprint/milestone), `--priority <level>`, `--feature <slug>`, `--objective <slug>`, `--since YYYY-MM-DD` / `--until YYYY-MM-DD` (inclusive, on `updated_at`), `-g/--group-by tag\|version\|priority\|status`, `--long`, `--tags`, `--json`. Filters compose (AND), case-insensitive. The snapshot's Active Tasks footer teaches this surface. |
| `tasks tags` | Distinct task tags with counts. `-a/--all`, `--json`. |
| `tasks create <name>` | Create a task. Flags: `-d/--description`, `-p/--priority critical\|high\|medium\|low`, `-u/--urgency …`, `-s/--status`, `-t/--tags <csv>`, `-w/--why`, `-v/--version`, `--person <name>`, `--reach <1-10>`, `--impact <1-5>`, `--confidence 25\|50\|75\|100`, `--effort <weeks>`, `--start YYYY-MM-DD`, `--due YYYY-MM-DD`, `--objectives <csv>` (roadmap objective slugs this task serves; slugs must exist), `--field <key=value>` (repeatable; sets declared custom fields), `--allow-missing-required` (create a draft even when a required custom field is unset), `--by human\|sleep` (who is filing; during a live sleep cycle anything but `human` must clear the filing bar), `--neighbor-checked <slug>` (lifts a review-band filing-bar refusal by naming the nearest task you looked at), `--declined-checked <key>` (lifts a semantic declined-match refusal by naming the declined idea you read; the exact-slug match is never lifted). **Fails** if a required custom field is unset and `--allow-missing-required` is not given. |
| `tasks decline "<topic>" --reason <text>` | Record an idea the user dropped that never became a task, so no sleep cycle re-files it. Reason mandatory (≥20 chars); key = slugified topic; refuses when a LIVE task with that slug exists (cancel that instead); a tombstoned slug may be declined. Writes `state/.task-declined.json` (synced brain content). |
| `tasks declined [--json]` | List declined ideas (newest first). |
| `tasks undecline <key>` | Lift a declined idea (awake decision), so it can be filed again. |
| `tasks tombstones` | List slugs deliberately retired by `tasks delete`/`rename` (with the task that absorbed each); a sleep cycle never re-files a tombstoned slug. |
...
| Command | Description |
|---|---|
| `lab create <slug>` | Scaffold `lab/insights/<slug>.md`. `--title <str>` (required), `--category <tab>` (top-level dashboard side-menu tab, e.g. "Marketing"), `--group <section>`, `--render <render>` (default number — the twelve above), `--size s\|m\|l` (board footprint: `s`/`m` = one column, `l` = two; omit to let the render decide), `--adapter http\|script` (default http), `--unit <str>`, `--ttl <minutes>` (default 1440). Edit the manifest afterwards to set the real endpoint/extract (or script) config, then sync. `--render funnel --adapter script` also scaffolds the `range` tweak (7d/28d/90d) + a documented `funnel-set/v1` script template (payload contract → tasks-and-features.md § Funnel insights); `--render app --adapter script` scaffolds the same `range` tweak + a documented `{ data, app }` script template — a `dataset/v1` bundle plus a multi-page bridge-driven body (contract → tasks-and-features.md § App insights). `--render breakdown` no longer scaffolds a script template (DEPRECATED — see the render table below); pass `--render app` for new dimensional/interactive work. |
...
| `lab show <slug>` | Manifest + cached series — **cache only, never fetches**. `--json`. Funnel insights additionally print per-funnel step tables (users, % of top, % of prev, drop) with the worst drop highlighted; breakdown insights print the pivot (rows × columns from the matrix dims, a third dim as sections, `*` marks low-sample cells); app insights print the declared page list (entry marked) + one pivot per named dataset in the `dataset/v1` bundle, and point you at `lab body`/`lab query` for the rest. |
| `lab body <slug>` | Read a script-authored card/app body FROM THE CACHE — **never fetches**. `--page <id>` (app/v1 only; default the entry page — an unknown id warns and falls back to entry), `--format text\|md\|html` (default text — `html` prints the raw source, `md` extracts headings/lists/tables), `--json` (`{ pages, page, html, text }`). Answers "what does this card actually show?" and lets an agent review or debug a body it did not author, without opening the dashboard. Works on both `app/v1` (page-scoped) and the single-body `html/v1` hybrid (`--page` is ignored there with a warning). |
| `lab query <slug>` | Slice/aggregate a cached `dataset/v1` dataset — **never fetches**. `--dataset <key>` (default: the bundle's `primary`, else the first), `--where key=value` (repeatable, exact-match filter), `--group-by <dim>`, `--top <n>`, `--date YYYY-MM-DD` (resolves to the dataset snapshot at/before that date — no interpolation, an honest "no snapshot" line when none qualifies), `--json`. Prints the same pivot style as `lab show`'s breakdown/app view. This is how an agent answers "what does this insight say broken down by X" from the terminal, on an `app` insight or any insight whose data is dimensional. |
| `lab report create <slug>` | Scaffold `lab/reports/<slug>.md` — a **My Reports** document composing EXISTING insights (a report owns no data and has no sync path). `--title <str>` (required), `--description <str>`, `--date-nav none\|daily\|weekly\|monthly` (default daily), `--insights a,b,c` (seed the first section; every slug must exist). Edit the manifest to add sections/items. |
| `lab report list` | All reports with section/item counts and date-nav mode. `--json`. |
| `lab report show <slug>` | The report with every item resolved under **window inheritance**: each item's target window is the date_nav span ending at `--date` (or today), unless a `window` spec pins it (`'own'` = the tile's own window / snapshot-as-of semantics). Prints "as of" + `window from→to [aligned\|window\|stale\|missing\|cannot]`; an unmeasured window → an honest "not measured yet" line pointing at `lab report sync` (never a substituted window, never interpolated). `--json` — each item includes `window`, `rangeKey`, `targetWindow`, `windowStatus`. |
| `lab report sync <slug>` | Fetch every window the report still owes (`--date YYYY-MM-DD` anchors them) into the **transient window cache** (`lab/cache/.windows/`) — the canonical caches, snapshot trails and KR bindings are never touched. Re-plans up to 3 passes; exits non-zero if windows stay owed. The right verb for automations that pre-warm a weekly report. |
| `lab report comment <slug>` | Generate the report's **AI commentary** for a view (`--date`, `--model`) — a pure-text headless `claude -p` run over the resolved numbers; the dated file lands in `lab/reports/.commentary/<slug>/`, always labelled with model + timestamp. Output contract: overall reading first, then `## <section title>` blocks that the report renders inside their sections. Run `lab report sync` first so it reads the right windows. Refuses on `commentary: false` reports (non-AI reports are first-class). |
...
**Renders (`--render`, twelve).** All read the same cached `Series[]`; the dashboard resolves each through its chart registry, so the CLI enum, doctor's validation and the board can't drift apart. `number` (latest value + inline sparkline) · `line` (trend over time) · `pie` (share by series — degrades to a bar list at ≥7 slices) · `bar` (latest value per series, horizontal) · `bar_compare` (grouped bars, series × last N buckets) · `stacked` (stacked bars over time) · `table` (series × latest + Δ, sortable) · `heatmap` (daily buckets as a week × weekday grid) · `raw` (the numbers, unstyled) · `funnel` (a multi-page render — a `funnel-set/v1` payload routes to `/lab/<slug>`) · `app` (a multi-page, bridge-driven render the SCRIPT builds itself — a `{ data, app }` return routes to `/lab/<slug>` and `/lab/<slug>/p/<pageId>`, full screen via `?fs=1`) · `breakdown` — **DEPRECATED, see below** (the dimensional pivot — a `matrix/v1` payload; 1 dim = bar list, 2 dims = heat-tinted pivot table, a 3rd dim = filter chips; existing insights keep rendering, no new scaffold). `table`, `funnel`, `app` and `breakdown` default to a 2-column card; `--size` overrides any of it. An unrecognised render reads back as `number` rather than blanking the board.

**Render DECISION TABLE — choose by the SHAPE of the data, and treat `raw` as the LAST RESORT.** A real board once shipped 22/54 insights as `raw` because 2-dimensional data was crammed into series NAMES ("F3000 · TR · 119k") — the contract was wrong, not the renders. Route by shape first:

| Your data is… | Render |
|---|---|
| One number that moves over time | `number` (card) or `line` (trend) |
| Several named series over time | `line` (few) · `stacked` (composition) · `bar_compare` (last N buckets) |
| A share/composition at one moment | `pie` (≤6 slices) or `bar` |
| One value per category, no time axis | `bar` |
| Series × latest value, scanned as a list | `table` |
| Daily intensity by weekday | `heatmap` |
| **Values broken down over 1-3 DIMENSIONS (funnel × language, plan × country)** | **`app` + a `dataset/v1` bundle in `data` — NEVER dimension values baked into series names.** (`breakdown` + `matrix/v1` still renders on EXISTING insights — DEPRECATED for new ones, see below.) |
| Sequential steps users fall through, compared across variants | `funnel` + a `funnel-set/v1` payload |
| A drill-down / multi-screen story over the data (comparison → detail, overview → breakdown) | `app` + `{ data, app }` — the script builds the pages itself, no component needed |
| Truly none of the above (an irregular payload a human must eyeball) | `raw` — the last resort, and a signal the data wants a typed contract |

A **snapshot** (all points share one date) is a shape smell: if it has dimensions it wants `dataset/v1` (the dataset history supplies the time axis, one dated snapshot per sync); a bare snapshot rendered `raw` at least drops its noise Time column automatically.

**`breakdown` / `matrix/v1` — DEPRECATED for new insights.** `matrix/v1`'s dimensional row×column grammar is now `dataset/v1`'s per-dataset shape (same dims/rows/total fields, delegated to the identical validator) — the successor is plural, so a multi-page `app` insight can give each page its own dataset. `lab create --render breakdown` no longer scaffolds a script template, and this table no longer routes new dimensional work there. **Existing `breakdown` insights keep rendering exactly as before — no migration is forced, nothing is removed.** The full contract (kept for reference, not new authoring) is still documented below under Breakdown insights.

**App bodies (`app/v1`) — a script builds its own multi-page, interactive, full-screen-capable UI.** A script returns `{ data, app }`: `data` is MANDATORY (typically a `dataset/v1` bundle — the queryable numbers, feeding `latest`, KR bindings, `lab show`, `lab query`); `app` declares an ordered list of PAGES, each a sandboxed html fragment, drawn in a network-less iframe (`sandbox="allow-scripts"`, no `allow-same-origin`, CSP `default-src 'none'` — same guarantee as `html/v1` below) and wired to the host over a postMessage bridge. **Inline `<script>` tags in the page html already run** — that has been true since `html/v1` shipped and nobody wrote it down; `app/v1` is what makes that interactivity *useful*, by giving the script a page to navigate to and real numbers to ask the host for. Contract, the `lab.*` bridge API, multi-page routing and full-screen → tasks-and-features.md § App insights.

**HTML card bodies (`html/v1` hybrid) — single-page, still supported.** A script may return `{ data, html? }` instead of a bare payload: `data` is MANDATORY and is exactly what a bare return would be (Series[], funnel-set, matrix, or dataset bundle — it keeps feeding `latest`, KR bindings, Δ, reports and `lab show`); `html` (≤300 KB, over-cap fails the sync loudly) is an OPTIONAL card body drawn in the same network-less sandboxed iframe `app/v1` uses. **Reach for a typed render first; write `html` (one page, no bridge) when the vocabulary can't say it in one screen, `app` when it needs more than one** — and when you do, **use the `lab-html-kit.css` classes (`lk-*`), don't write your own CSS**: the kit (title/value/label/delta/stat/chip/table/bar-row + low-sample helpers) plus the injected design tokens make the card look native dreamcontext in both themes. **Height is automatic on both** — the body measures itself and the host resizes to match; the only bound is the board grid's (a CARD preview is clamped to 120–320px, the detail/page view is not), and there is no height field in the manifest. Never shrink type below the kit's scale to fit a card: a card over the cap reads in full when opened, and a body needing more than one screen is what `app` is for. Contract + class list → tasks-and-features.md § HTML card bodies.
