...
| `lab report show <slug>` | The report with every item resolved. `--date YYYY-MM-DD` resolves each item to the **latest snapshot at/before the end of that date** and prints its "as of" sync stamp; no qualifying snapshot → an honest "nothing to show" line (never interpolated). `--json`. |
