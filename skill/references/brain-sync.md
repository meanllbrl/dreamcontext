...
- **`state/.brain-local.json` — MACHINE-LOCAL** (gitignored). Runtime bookkeeping: `lastFetchAt`, `pulledUpdates`, `pendingAgentMerge`, `needsTaskSync`, `codeRepoPath` — **plus `activePersonSlug`**, this machine's person pin (`dreamcontext people whoami --set <slug>`). This is what keeps two machines from clobbering each other's sync state, and it is why **machine identity never enters the synced brain**: the roster is shared, who is at *this* keyboard is not.
- **`people/people.json` — SYNCS**, and merges as a key **union** (see Conflicts below). `people/<slug>.md` sync as ordinary prose.
- **Retired:** the `people` roster key in `.config.json` was removed in 0.23.0. The merger deletes it unconditionally, so a pre-0.23 teammate cannot re-introduce a ghost roster. (`peopleIdentity` — the ClickUp/GitHub id side-table — is untouched and still unions.)
...
**People merge in two different lanes.** `people/people.json` is **deterministic**: the CLI merges it as a **key UNION** across machines (`version` = max, roster keys unioned, ours wins on a shared slug, `emails[]` unioned) — because two teammates adding two different people must not lose one of them. **Caveat: there is no tombstone.** `people rm` deletes a row locally, and a teammate whose roster still lists that person **resurrects them on the next sync** — remove them on both sides. `people/<slug>.md` (the constitutions) ride the **`'other'` prose lane** instead: one writer per file makes real conflicts rare, and when they do happen they resolve as prose, exactly like a knowledge doc (`/dream-sync` reads base/ours/theirs and writes the merge). `state/.brain-local.json` — which holds this machine's active-person pin — is gitignored and never travels.

**Conflicts:** deterministic files — JSON (changelog/releases/config/taxonomy/**people**) and task `.md` — are auto-merged by the CLI. Only overlapping edits to the **same `##` section of a knowledge/feature (prose) doc** defer to the agent (`awaiting-agent`): a report with base/ours/theirs snapshots is written to `state/.brain-merge/`, the `/dream-sync` skill writes the real merge, and `brain sync --continue` commits it. Real **code** conflicts go to the human with native git markers (never to the agent). A **scrub gate** runs before every commit and push (BLOCK always stops it; WARN stops only under `--strict` or a headless pull-only).
