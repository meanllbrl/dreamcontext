...
| **✅ Team brain sync (shared repo)** | **Yes — a team OR one person across machines can share ONE brain.** The brain is git-synced to a GitHub remote in one of **three modes** — `separate` (brain in its own repo), `full-repo` (whole project → its `origin`), `in-tree` (commit-only default). `sleep done` auto fetch→merge→commit→pushes it; the `/dream-sync` skill resolves prose conflicts. Different from federation (read-only cross-project recall) and cloud task sync (tasks only). Each machine needs its own token. | [brain-sync.md](references/brain-sync.md) |
...
- **Team collaboration / shared brain / second machine** — when the user wants to **use one brain with other people or on another computer** ("share the brain", "collaborate on tasks/knowledge together", "put the brain in its own repo", "sync my brain to GitHub", "set it up on my other machine"), that's the **shared brain repo**: `dreamcontext brain init` (create) / `brain attach <url>` (join) / `brain status`, auto-synced at `sleep done`, prose conflicts resolved by the **`/dream-sync`** skill. **Guide them into it — don't say "unsupported".** Three modes (separate / full-repo / in-tree), per-machine token, and cross-OS setup → [brain-sync.md](references/brain-sync.md).
...
- **[brain-sync.md](references/brain-sync.md)** — **shared brain repo / GitHub cloud sync**: the three modes (separate/full-repo/in-tree), setup, per-machine token + auth, shared vs machine-local config, lifecycle, **cross-OS setup**, and the silent-failure troubleshooting playbook.
