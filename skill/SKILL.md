...
| **Web dashboard** | Local React UI: Kanban, Eisenhower matrix, brain graph, sleep tracker, council hall, in-app Claude Code agent (terminal; **Chat view BETA** — native markdown/tool/question cards, same engine, and the agent knows it: boards drawn inline, media playable, backticked paths clickable, `dream-actions` buttons) | [integrations.md](references/integrations.md) |
