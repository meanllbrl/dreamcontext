...
Council skill built and in review (task: council-skill, status: in_review). Full implementation in `src/cli/commands/council.ts` (14 sub-commands) + skill pack at `skill-packs/council/`. Council UI shipped on dashboard (v3: CouncilHall grid + CouncilDetail full-page with Overview/Agents/Matrix tabs). Brain graph (Obsidian-style, react-force-graph-2d) also shipped with light/dark mode support. New `dreamcontext install-claude-md` command available. Next: optional-skill-packs Phase 3 (community skills), Dashboard Phase 5 (a11y, responsive, i18n tokens, bundle audit) before v0.2.0 release.
