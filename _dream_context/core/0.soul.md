...
v0.2.0 shipped (2026-04-25). Contains: Council skill (14 CLI sub-commands + CouncilHall/CouncilDetail dashboard UI), Brain graph (Obsidian-style, react-force-graph-2d, light/dark mode), install-claude-md command, optional-skill-packs Phase 2. Active work: meta-marketing-skill PR 0.5 (agent training from competitor corpus — 9 videos ingested, 2 delta signals extracted, skill-pack files blocked on 8 open user decisions). Next milestones: resolve meta-marketing-skill user decisions → generate skill-pack files → PR 0.5 sign-off; then meta-marketing-skill PR 1 (metaFetch + typed client). Dashboard Phase 5 (a11y, responsive, i18n) still pending.
...
- **15 commands, 7 hooks**: bookmark, trigger, transcript, council (14 sub-commands), install-claude-md added. Register new commands in `src/cli/index.ts` and add to SKILL.md command reference table. Hooks: SessionStart, Stop, SubagentStart, PreToolUse, UserPromptSubmit, PostToolUse (fires on Edit|Write, 30s), PreCompact (5s) — all registered via `install-skill`. `ensureHooks()` is now data-driven via HOOK_SPECS table; adding a new hook is a one-line change. Council commands (14 sub-commands in `council.ts`) are implemented and in review.
