...
Project renamed from `agentcontext` to `dreamcontext` (npm package, binary, GitHub repo, folder). Rename complete. Optional skill-packs system Phase 1 + Level 2 complete (42 files: engineering, design, growth, brand-voice packs + standalone system-prompts + 6 agents). Next: Phase 2 of optional-skill-packs task — extend `install-skill` CLI with `--packs` / `--skill` options. After that: Dashboard Phase 5 (accessibility audit, responsive layout, i18n tokens, bundle audit) before v0.2.0 release.
