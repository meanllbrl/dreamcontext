...
- `src/server/routes/{config,packs,version-check,vaults}.ts` — the four control-plane routes (vaults route has GET + POST + DELETE). Registered in `src/server/index.ts`; CSRF/CORS pipeline unchanged.
...
- Pages: `SettingsPage`, `PacksPage` (+ CSS); component `UpdateBadge` (mounted in `Header.tsx`).
