...
description: 'Multi-agent code review skill with router, niche specialists, coordinator'
priority: medium
urgency: medium
status: in_review
...
Productize the multi-reviewer team pattern: router classifies diffs by tier + domain, dispatches niche specialists (security, cloud-functions, frontend, edge-cases) in parallel, coordinator dedupes findings. Distinct from the existing single reviewer agent (too generic for non-trivial diffs) and from the pre-implementation three-reviewer-parallel-mandates pattern (different stage).

## User Stories
<!-- As a <role>, I can <action>, so that <outcome>. Tick when demonstrably true in the running system. -->

- [x] As a developer, I can run /multi-review on a branch diff so that a team of niche specialists review it in parallel and a coordinator surfaces the unified verdict.
...
- [x] .claude/agents/review-coordinator.md shipped — sole reader of all full reports, dedupes/re-ranks/drops false positives, emits unified verdict (READY_TO_MERGE / NEEDS_ATTENTION / NEEDS_WORK)
- [ ] Tested on a real non-trivial diff — router classification + coordinator output validated in practice
## Constraints & Decisions
<!-- LIFO: newest at top. Capture the why, not just the what. -->

- **[2026-05-24]** Coordinator is the only agent that reads full specialist reports — main agent only reads executive summaries (council pattern isolation)
