...
status_pr0: shipped
status_pr0_5: >-
  COMPLETE · 9 videos ingested + learnings written · skill-packs/meta-marketing/ fully generated
  (SKILL.md + 5 reference files) · all 8 user decisions resolved · no open questions remaining
status_pr1_8: todo
tags:
  - skill
  - marketing
...
**Status as of 2026-04-25 (PR-0.5 COMPLETE):** PR 0 shipped. PR 0.5 complete. 9 YouTube videos ingested. `skill-packs/meta-marketing/` fully generated. All 8 user decisions resolved. Working tree has ingester patches uncommitted — those ship in PR 0.5 commit.

### What PR 0.5 delivered (complete)

**Ingester patches (working tree, uncommitted — commit with PR 0.5):**
- `tools/reinfluence/__main__.py` + `_dream_context/marketing/.tools/reinfluence/__main__.py` — YouTube path now uses `youtube-transcript-api` (no PO-token gate, no Whisper, no video download, no frames).
- `tools/reinfluence/requirements.txt` — added `youtube-transcript-api>=1.0`.

**skill-packs/meta-marketing/ (generated this session):**
- `SKILL.md` — main operator skill file (5 sections, 2 hard blocks, Trust-Meta principle, pre-scale gate, testing methodology resolved)
- `account-ops.md` — Tier 1/2 rules from corpus (4 speakers, all conflicts resolved or documented)
- `copy-formulas.md` — callout/agitation/benefit/scarcity/CTA + hook-swap formula
- `creative-frameworks.md` — 4×3×3 grid + "on us" reciprocity hack (Tilki-specific)
- `mistakes.md` — 12 failure modes from external report, cross-referenced with corpus
- `platform-state.md` — time-stamped Meta UI facts (separate from operator rules, updatable)
...
PR 0.5 is complete. The skill-pack is live at `skill-packs/meta-marketing/`. Next steps are PR 1 (agent roster) and committing the working tree.

1. **Greet the user, confirm state loaded, then ask:**
   - Commit all PR 0.5 work (ingester patches + 9 learnings files + skill-pack) as a single commit, OR
   - Continue ingesting more corpus videos (same flow — verdict + signal/noise, wait for "confirmed" before writing learnings), OR
   - Start PR 1: build the agent roster (`skill-packs/agents/marketing-strategy.md`, `marketing-monitor.md`, stubbed `marketing-creative.md`)
2. **Before any new learnings file:** preview verdict + recommendation, wait for "confirmed" before writing.
3. **Same-speaker discipline still applies:** Ben (4 videos), Charlie (1), Moonlighters (1), Optimizer (1). Any new Ben URL = delta extraction only.
4. **Omnipresent content gate:** before recommending any campaign structure for budgets above ₺30-40K/month, ingest Ben's omnipresent content video first.
...
**PR 1 — Meta foundation (`metaFetch` + typed client).**
- `meta-fetch.ts` (retry, backoff, idempotency, token-expiry, chunked upload, ctx-threaded dry-run gate, header-only auth).
- `meta-client.ts` typed surface.
- Unit tests against fake Graph mock.

**PR 2 — CLI surface (read + safety verbs).**
- `mk init`, `account list/use`, `cohort create` with hypothesis shape-validation, `today`, `diff`, `insights pull` (sync 24h, 15-min cache), `pause`, `resume`, `scale --pct`, `kill --bottom`, `doctor`.
- Tab-completion script + `mk` alias.
- All mutations dry-run default; `--no-dry-run` flag wired.
...
**PR 6 — Sub-agents (Strategy Optimizer + Performance Monitor) — grounded in PR 0.5 training corpus.**
- Distributed via `install-skill`. CLI-first; library imports forbidden.
- CreativeDirector stub file shipped, gated by `config.json: marketing.creative_director.enabled = false`.
