...
### 6. A report nobody acknowledged is a report that never arrived — give the host a way to ASK (2026-08-27)

Chat's height bridge was one-way and fire-and-forget: the frame measured its body, posted one number, and deduped against its last value. Two independent things then had to be true for the block to be sized at all — the host's `message` listener had to already exist, and the body had to keep changing size if it didn't. Neither is guaranteed. The listener was attached in a `useEffect`, and passive effects flush AFTER paint, which under a streaming turn can land after the frame's document has already parsed, run the bridge and spoken. A settled body never resizes again, so the dedupe had nothing more to volunteer. One missed message = a block frozen at its 40px floor for the rest of the session, with its content drawn perfectly and clipped.

The rule: **a one-shot child->host report is only as reliable as the host's attach timing, so it is not reliable.** Three moves, each covering a different half of the race:

1. **The host listens in `useLayoutEffect`, never `useEffect`** — layout effects run synchronously inside the commit, before the browser can run the frame's parser task. Passive effects race it.
2. **The host can ask, and the answer is FORCED past the child's dedupe** (`__dreamHtmlMeasure` → `report(true)`). A deduped bridge answers a retry with silence, which is the same as being broken. Ask on the iframe's own `load`, then on a short interval until an answer lands, with a finite cap (a frame whose bridge never ran will never answer).
3. **Never key host state on a measurement reset by a passive effect.** Store the measurement WITH the body it measured (`{html, height}`) and derive; a reset effect running after paint can otherwise clobber a height the new document already delivered.
