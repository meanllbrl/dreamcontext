...
- `dashboard/src/components/agents/AgentsFeed.css` `.agent-msg-question-choice--alt` (2026-09-25): a run's question offers two buttons, and two filled accents said "both are the default". The first choice keeps the strong fill; every other choice takes this recipe (accent 14% into `--color-bg-tertiary`, 45% into the border, `--color-text` label), so the one accent on the screen means "the recommended answer". A `color-mix()` tint is not an accent fill to the allowlist test, so no entry was needed.
- `dashboard/src/styles/tokens.css` `--color-success-ink` / `--color-error-ink` and `chat/atoms.css` role discs (2026-09-25, quest-party Chat): the team log writes status as TEXT ("Reading…", "Couldn't edit …"), so it takes ink tokens, not the success and error swatches, and the inks are measured on every surface a row sits on. The dark error ink had to be lighter than its swatch: `#ff5a5f` read 4.38:1 on dark `--color-bg-tertiary`, `hsl(0 100% 72%)` reads 4.96:1; the verify gate also measures it on `--color-error-subtle`, the tint a failed row inside a work beat sits on. Role avatars and verdict chips follow the recipe: the role hue at 22% into `--color-bg-secondary` for the disc (55% for the border) with the face in `--color-text`, and verdicts at 12% success or error with `--color-text` labels, never a filled hue.
