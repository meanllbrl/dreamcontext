...
| **Keep injected patterns true** (patterns-auto-injection, 2026-09-10) | UserPromptSubmit pins the directive in `src/cli/commands/hook.ts` alongside the injected pattern block — a contradicted, narrowed or extended pattern is edited in place **before the task finishes**, with the change named in the reply; the text is code and locked by test, so it cannot drift from the injection it governs | SKILL.md Entity Router carries the one-liner on the Pattern row; `references/knowledge-and-recall.md` § Patterns carries the full explanation |

**The 4th application also shows the pattern's corollary: when the hook takes a rule, something else must give it up.** The keep-them-true rule previously had no owner at all — the assumption was that sleep would fold user corrections into patterns. Placing it in the hook made that assumption not just redundant but wrong, so `sleep-product` was **narrowed to match**: it keeps pattern creation, condensation, retirement and shim sync, and is now forbidden from folding a user correction into a pattern. By the time a sleep cycle runs, the argument that produced the correction is gone and only its conclusion survives — the correction has to be written in the turn where it was said. A cycle that finds a contradicted-but-unupdated pattern reports it as a missed awake rule instead of silently rewriting it. Routing a rule to a hook without removing its old owner leaves two writers for one file and a race over which one is right.
