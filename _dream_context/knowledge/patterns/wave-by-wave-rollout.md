...
### Occurrence 3: automations redesign (2026-08-10/11, 24 tasks across 11 waves)

Tasks `automations-redesign-the-flow-graph-is-the-manifest-the-run-asks-in-chat-and-the-queue-never-loses-a-fire` + `automations-hitl-a-run-can-stop-and-ask-and-the-verdict-comes-back-from-wherever-you-are`. Final uncommitted diff: ~+2613/-2004 lines across 18 files. A shipped capability (the review-queue model, only five commits old) was **retired and deleted** — `review.ts`, `card-registry.ts`, `AutomationsReviewQueue.tsx/.css`, and their tests — replaced by flow-graph architecture.

- **W1-W3** (contract root): types, queue, session-registry, store flow parse, registry hash, flow-registry, hitl, verdict repoint, flow-runner pure, telegram additive-only. All behavior-neutral — new types compile green, `flow` reads leniently toward `null` (no flow = pre-redesign), approval hash omits-when-null so pre-redesign manifests keep exact hash. Gate: tsc + tests green + 44/44 `verify:automations` both themes.
- **W4-W5** (behavior flip): runner lifecycle + flow execution, fourth SessionKind, tick drain + telegram cutover, server routes. Gate: same.
- **W6-W8** (UI surfaces): AutomationFlowCanvas, hooks + queue removal, AgentSurface + agent-sessions, ChatPane inline question card, agent-chat gate. Gate: same + dashboard build clean.
- **W9** (deletion): snapshot repoint, **delete `review.ts`/`card-registry.ts`** (six importers repointed themselves in prior waves), test sweep. Gate: same + grep proof (zero references to deleted files).
- **W10** (verification): `scripts/verify/automations.mjs` runtime proof, feature-integration checklist.

**Lessons from the full run (W1-W10, 2026-08-10 → 2026-08-11):**
- **Nine of eleven waves needed a resume** — implementers returned completion-shaped text with the work half-done. Verifying via `git status` + test counts rather than trusting the report caught it every time: a missing test file, an unchanged test count, an unused fixture. **Lesson:** when an implementer reports done, CHECK `git status` for expected files and `npm test | grep passing` for expected count delta before marking the wave complete.
- **Deletion must be LAST when the deleted module has importers.** `review.ts` had six importers — verdict, telegram, runner, routes/automations, cli/commands/automations, snapshot. Each repointed itself in its own wave; T12 (W9) finished the job and owned the grep proof. Deleting earlier breaks the build for four waves. **Lesson:** when retiring a module, map its importers first; each repoints itself in a prior wave; deletion comes last with zero-reference proof.
- **Runtime proof found defects unit tests did not.** The headless verification script (`verify:automations`) found three real defects that 6,944 unit tests missed: (1) create-chat bridge had no subscriber (T19 built it, listener belonged in T18a's file, neither owned the wire); (2) a test passing for the wrong reason (baseline captured before a silently-failing step — fixing the product broke the test); (3) `chatPanes` filtering on the display `kind` so every automation-run tab spawned a healthy ChatSession that was never mounted (one field double-booked between tab glyph and pane mount). **Lesson:** runtime measurement verification earns its cost — headless end-to-end catches wiring gaps unit tests cannot see.
