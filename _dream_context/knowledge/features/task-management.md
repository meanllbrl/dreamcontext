...
- [x] LOCAL-ONLY field registry (`src/lib/task-backend/local-only.ts`): `REMOTE_MAPPED_TASK_FIELDS` is the CLOSED set a pull may author (name, status, priority, urgency, tags, version, start_date, due_date, updated_at/by, created_by, custom_fields, assignee, source_project). Everything else — including a key this codebase has never heard of — is local-only by complement, so the rule fails SAFE. The audit: `objectives`, `rice`, `parent_task`, `related_feature`, `description`, `id`, `created_at` are local-only on GitHub; `start_date`/`due_date`/`tags` are NOT (GitHub carries them in the `dc:dates` body block + labels; ClickUp natively). On ClickUp, `description`/`related_feature`/`rice`/`urgency` become remote-owned only when the list actually binds a matching custom field — the guard takes those keys per-merge (`alsoRemoteOwned`), so clearing a bound field from ClickUp stays a legitimate merge outcome.
- [x] `preserveLocalOnlyFields(localFm, mergedFm, alsoRemoteOwned)` runs in BOTH pull merge paths right before the mirror write: it restores any local-only field the merge emptied and RETURNS the rescued names, which the sync report surfaces as a warning. A mapping regression can therefore never be silent again.
- [x] Vanished-mirror REBUILD: the pull's create branch also fires when a mapped task's file is gone (lost `state/` file, `.tasks-map.json` team-merge re-slug). It now merges `localOnlyFieldsFromSnapshot(base_snapshot)` into the remote-derived frontmatter — objectives/rice/parent_task/related_feature come back instead of being reset to null — and warns `REBUILT`, naming what it restored. Identity (`id`, `created_at`) is still minted fresh.
- [x] PRE-PULL BACKUP: every mirror a pull overwrites or deletes is copied first to `state/.pre-pull-<stamp>/` (one dir per run, newest 5 kept, gitignored via `REMOTE_BACKEND_GITIGNORE_ENTRIES`), and `SyncReport.mirrorBackupDir` carries the path so `tasks sync` prints it. `hard-refresh` already backed up; an ordinary pull did not.
- [x] POISON-PILL WATERMARK: `SyncLedger.noteUnapplied()` / `enforceUnappliedFloor()` cap the global pull watermark strictly below the oldest remote item the run did NOT ingest (a task that threw, or one left unprocessed by a lock-lost abort). `advanceWatermark` clamps forward, `enforceUnappliedFloor` retreats an already-written watermark — needed because the batch is not time-sorted, so the failure can surface after a newer item already advanced it. In-memory and per-run (a failure is transient by assumption).
- [x] TAG-RETRY (ClickUp): the per-tag add/remove loop is tolerant — an op that fails is recorded, never fatal, and the remaining tags still get their chance; a 404 on a REMOVE is the already-applied case, not a failure. On a genuine failure the new `base_snapshot` keeps the PREVIOUS tag set (everything else is absorbed, so no duplicate changelog comments), `pendingPush` stays true, and a `push` op is re-queued AFTER the dequeue so the delta is actually retried.
- [x] `doctor` link-loss check (`src/lib/roadmap-link-audit.ts`, code `doctor/objective-links-vanished`): compares the committed `knowledge/roadmap/board.md` against live task frontmatter and warns when an objective's ENTIRE recorded link set is gone while the tasks still exist. Deliberately narrow — a partial unlink is a legitimate PO action.

### Date range auto-reschedule + stamped-date validation (PR #296, v0.24.0)
- [x] Moving a start past the due date now auto-reschedules the due date, preserving the window's original length (a five-day job that starts late stays a five-day job), instead of hard-rejecting the move. All date mutation paths (`tasks start`, `tasks due`, dashboard PATCH route, Gantt drag) route through one module (`src/lib/task-backend/task-dates.ts`: `normalizeDateRange()`) so the rules cannot drift between clients.
- [x] Completing a task stamps the REAL end (`completed_at` = today) so the timeline shows what happened rather than what was planned, regardless of the original `due_date`. Isolated in `shouldStampCompletedAt()` helper; only the first transition to `completed` stamps.
- [x] Stamped dates (auto-filled `start_date` on first `in_progress`, auto-filled `completed_at` on first `completed`) are now validated like typed dates — `isCalendarDate()` check, start≤due≤completed constraint. Previously only explicitly-set dates were validated, so an invalid auto-stamp could silently land in frontmatter.
- [x] **Known and NOT fixed in PR #296**: `isCalendarDate` accepts impossible days (2026-02-30, 2027-02-29, 2026-04-31 all pass — V8 rolls them forward to the next valid date). A task with start 2026-02-30 displays as 2026-03-02. Not a blocker; noted for future hardening.
...
### Task-sync integrity + repair (v0.19.0)
- [x] `tasks dedup [--dry-run] [--yes]`: LOCAL-ONLY repair command heals duplicate task families (`state/<slug>-2/-3/-4.md`) created by corrupted/conflicted `.tasks-map.json` — merges each family to its canonical slug, repoints the ledger, removes redundant files. Dry-run reports the plan; apply requires `--yes`. Never constructs a remote adapter.
- [x] Dedup family merge: picks the newest body (by `updated_at`, mtime tiebreak), unions all Changelog sections, and maps the merged file to the remoteId shared by any mapped member. Families with members mapped to DIFFERENT remoteIds are skipped (never merged).
- [x] Dedup map repair: detects dcId↔remoteId cross-wiring across the whole ledger and reports each finding with its disposition (`repaired` | `unrepairable` | `dcId-collision`). Repaired cross-wires repoint on apply; unrepairable/collision entries are flagged but left as-is.
- [x] Pull-fallback duplicate guard: name/id match in `clickup.ts`/`github.ts` is guarded by `base_snapshot` — a never-synced local creation (no ledger entry, no sync history) ≠ a genuine #204 orphan (corrupted map, intact `.tasks-sync.json` history). Only entries with `base_snapshot` set are fallback-matched.
- [x] Semantic-merge `.tasks-map.json` deterministic resolution: new `tasks-map-json` MergeClass in `semantic-merge.ts` resolves the ledger via union merge (`unionTaskMap` from `sync-state.ts`), never deferred to the agent. Conflict markers never survive in the committed map.
- [x] Task-md add/add collision handling: when both sides add a task at the same slug with distinct dcIds, the smaller dcId's file is kept verbatim at the bare slug and the larger dcId's file becomes a `-N` sibling. Both files reach the working tree; the agent never has to choose which to drop.
- [x] Conflict-marker parsing (`conflict-markers.ts`): `splitConflictMarkers` is a pure text scan that reconstructs `ours`/`theirs` from literal git conflict markers baked into committed file bytes (the fallback when no merge is in progress and no index stage exists).
...
- **[2026-08-17]** **A pull owns a CLOSED set of fields; everything else is local-only by complement.** The 2026-07-27 loss (72 task→objective links stripped by one `tasks sync pull`) was not a mapping bug — `objectives` was simply *unknown* to the pull path, so nothing guaranteed it survived. The fix is structural rather than per-field: `REMOTE_MAPPED_TASK_FIELDS` enumerates what a backend may author, and `preserveLocalOnlyFields` restores anything else the merge emptied. An unknown key therefore fails SAFE (preserved), which is the only rule that survives the next feature adding a frontmatter field. Three companion decisions: (1) the guard NAMES what it rescued in `SyncReport.warnings` — a rescue means the mapping regressed, and silence is what made the original loss expensive; (2) a vanished mirror is REBUILT from remote + the ledger's `base_snapshot` (the last known local bytes) rather than remote-only, because the create branch is reachable for tasks that already exist — a `.tasks-map.json` team-merge re-slug materialises exactly that shape; (3) `state/` is gitignored, so **every pull now backs up the mirrors it changes** to `state/.pre-pull-<stamp>/` (newest 5 kept) and the sync prints the path — `hard-refresh` already had a backup, an ordinary pull did not, and "the committed board.md happened to still list the links" is luck, not a recovery path. Detection closes the loop: `doctor` compares the committed board against live task frontmatter and flags an objective whose whole link set vanished while its tasks live on.
- **[2026-08-17]** **The pull watermark may never pass an item the run did not ingest.** The batch is listed in remote order, not time order, and the watermark advances per APPLIED item — so one task that throws was dropped from every future delta pull the moment any later task succeeded with a newer timestamp (same silent-exclusion family as #185, and the pull still reported success). The floor (`noteUnapplied` + `enforceUnappliedFloor`) is in-memory and per-run: it clamps forward advances AND retreats a watermark already written past a failure discovered later. Scoped deliberately to failures and abort-leftovers — a ClickUp `foreign` row or a locally-deleted task is skipped *permanently and by design*, and everything above the watermark is fetched in the same batch anyway, so those cases cannot hide an unpulled task and must not pin the delta window open forever.
- **[2026-08-17]** **A partially-applied tag loop must be retryable, not wedged.** ClickUp's PUT carries no tags, so a tag change is one request per tag and can die half-applied. Aborting (the old behavior) left `base_snapshot` pre-push, so the retry replayed ops that had already landed. Now the loop tolerates failure per-op (a 404 on a REMOVE *is* the already-applied case), and on a genuine failure the new base keeps only the OLD tag set: everything else is absorbed — so the retry cannot duplicate changelog comments — while the tag delta recomputes and is re-queued after the dequeue. Replay safety is by construction, not by trusting the provider: an add of an existing tag / a remove of an absent one is a no-op, and if that ever changes the error is caught and reported instead of wedging the push.
- **[2026-07-22]** Task-sync integrity suite (v0.19.0) — repair over rebuild: `tasks dedup` is the sanctioned path for duplicate-family cleanup; hand-deleting `-N` files or hand-editing `.tasks-map.json` bypasses the ledger integrity checks and can corrupt the sync state. The command is LOCAL-ONLY by design — no adapter is ever constructed, so cloud-backend credentials are never touched. Dry-run is mandatory culture (`--dry-run` first, always); the mutating run requires `--yes` mirroring `tasks delete`'s idiom. Conflict-marker healing in `dedup.ts` uses an in-memory union (`readEffectiveMap` → `splitConflictMarkers` → `unionTaskMap`) during planning so `--dry-run` never writes; the physical heal write (`ledger.healConflictedMap()`) happens only inside `applyDedup` on a confirmed apply. The semantic-merge `.tasks-map.json` union merge is deterministic (never deferred to the agent) because the ledger is a pure index — no prose to reconcile, just set union. Task-md add/add collision keeps BOTH files (smaller dcId verbatim, larger → `-N` sibling) rather than forcing the agent to choose, so no work is lost. The pull-fallback `base_snapshot` discriminator distinguishes a never-synced local creation from a genuine #204 orphan (committed map corrupted, machine-local `.tasks-sync.json` intact) — every `updateTaskSync` call in both backends writes `base_snapshot`, so the presence/absence signal is reliable.
...
**Task-sync integrity + repair (v0.19.0)** (`src/lib/task-backend/dedup.ts`, `conflict-markers.ts`, edits to `semantic-merge.ts`/`sync-state.ts`/`clickup.ts`/`github.ts`):
- `dreamcontext tasks dedup [--dry-run] [--yes]` — LOCAL-ONLY repair command. Scans `state/*.md`, groups by base slug (strips `-\d+$` suffix), picks the newest body per family (by `updated_at`, mtime tiebreak), unions all Changelog sections, and merges each family to its canonical slug. Skips families whose members map to DIFFERENT remoteIds (genuinely distinct remote tasks). Detects dcId↔remoteId cross-wiring across the whole ledger and reports repairs (`repaired` | `unrepairable` | `dcId-collision`). Dry-run computes the full plan in memory (including healed map via `readEffectiveMap` → `splitConflictMarkers` → `unionTaskMap`, no write); apply requires `--yes` and commits the repairs (file merge, ledger repoint, physical map heal via `ledger.healConflictedMap()`, file remove).
- `conflict-markers.ts` — `splitConflictMarkers(text)`: pure text scan that reconstructs `ours`/`theirs` from literal git conflict markers (`<<<<<<<` / `=======` / `>>>>>>>`), handles multiple hunks, optional diff3 `|||||||` base section (discarded), and arbitrary branch labels. Returns `null` when no markers present. The fallback when no merge is in progress and no index stage exists.
- `semantic-merge.ts` — new `tasks-map-json` MergeClass: `classifyPath` routes `state/.tasks-map.json` (with/without `_dream_context/` prefix) before the `state/*.md` rule; `mergeTasksMapJson` parses both sides tolerantly, unions via `unionTaskMap`, re-serializes as 2-space JSON + trailing newline; `resolveConflicts` stages it deterministically, never `deferredToAgent`. Task-md add/add guard: when `base.trim() === ''` AND both sides have a string `id:` AND they differ → smaller dcId's raw text kept verbatim as `merged`, larger's raw text as `sibling.content` (next free `-N` via `existsSync`). Every other case (same dcId, missing id, real base) falls through, `sibling: null`. `reorderTaskMdFirst` sorts task-md paths ascending so multi-family sibling-suffix assignment is input-order independent.
- `sync-state.ts` — `unionTaskMap(a, b)`: set-union of distinct entries (by dcId), same-remoteId collapse (deterministic dcId win), sorted output, no NUL in `entryOrderKey` (separator changed from `\x00` to `|` so the map file is a text diff, never binary). `splitConflictMarkers` imported + called before every ledger read where markers may exist.
- `clickup.ts` / `github.ts` — pull-fallback name/id match now guarded by `if (match && this.ledger.taskSync(match.slug)?.base_snapshot)` — a never-synced local creation has neither a map entry nor `.tasks-sync.json` history, while a genuine #204 orphan (committed map corrupted, machine-local sync history intact) always has `base_snapshot` set (every `updateTaskSync` call writes it). The guard prevents false-positive matches between a new local task and an unrelated remote orphan.
- CLI (`src/cli/commands/tasks.ts`) — `dedup` verb (globs state dir, calls `planDedup` / `applyDedup`, prints the plan with color-coded disposition, exits 0 on dry-run or 1 on no-op apply). Skill docs (`skill/SKILL.md`, `references/integrations.md`, `references/cli-reference.md`) already updated by the implementer (Task E).
...
### 2026-08-17 - The pull path stops losing data silently (v0.24.2)
- Closed the 2026-07-27 loss (72 task→objective links stripped by one `tasks sync pull`) structurally: new `local-only.ts` registry says what a backend MAY write; everything else is preserved by complement, so an unknown frontmatter key fails safe. Wired into both pull merge paths; a rescue is named in `SyncReport.warnings`, never silent.
- A vanished mirror is now REBUILT from remote + the ledger's `base_snapshot` (local-only fields restored, `REBUILT` warning) instead of being reset to a remote-only skeleton — the shape a `.tasks-map.json` team-merge re-slug produces.
- Every pull backs up the mirrors it overwrites/deletes to `state/.pre-pull-<stamp>/` (newest 5, gitignored) and prints the path; `state/` is gitignored, so this is the only recovery path an ordinary pull ever had.
- Poison-pill watermark fixed: the global watermark can no longer pass a task that failed to apply (or one an aborted run never reached) — `noteUnapplied`/`enforceUnappliedFloor` clamp forward and retreat, because the batch is not time-sorted.
- ClickUp tag loop made replay-safe: per-op tolerance (404 on a REMOVE = already applied), base snapshot keeps the OLD tag set on a genuine failure so the delta retries without duplicating changelog comments, and the task is re-queued after the dequeue.
- Detection: `doctor/objective-links-vanished` compares the committed `knowledge/roadmap/board.md` against live task frontmatter and warns when an objective's whole link set disappears while its tasks live on.
- Tests: +20 `sync-local-only-fields`, +7 `sync-watermark-poison-pill`, +3 `clickup-tag-retry`, +10 `roadmap-link-audit`, +2 `clickup-pull`. Full unit suite green.

### 2026-07-22 - Task-sync integrity + repair suite (v0.19.0, in working tree)
- Seven new acceptance criteria (all ticked) for task-sync integrity: `tasks dedup` LOCAL-ONLY repair command (dry-run + `--yes` apply), family merge logic (newest body, union Changelog, skip distinct-remoteId families), map cross-wiring repair, pull-fallback `base_snapshot` guard, semantic-merge `.tasks-map.json` deterministic union, task-md add/add collision keep-both, and conflict-marker parsing utility.
- Technical details extended with full `dedup.ts` / `conflict-markers.ts` / `semantic-merge.ts` / `sync-state.ts` / backend guard contract. Constraints/decisions document the repair-over-rebuild philosophy, in-memory dry-run heal, LOCAL-ONLY guarantee, and `base_snapshot` discriminator reliability.
- Skill docs (`SKILL.md`, `integrations.md`, `cli-reference.md`) already updated by the implementer (Task E, session af0aa899).
- Status stays `in_review` — the work is uncommitted and pending integration testing + a post-consolidation review.
