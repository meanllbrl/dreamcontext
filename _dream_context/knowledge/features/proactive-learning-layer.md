...
- LIST VIEW RETIRED (7f1a539): owner review "çok kötü". `ThesisListView.tsx`, toolbar List/Board toggle, `view`/`listFilter` prefs, `ThesisViewMode`/`ThesisListFilter`, `needsAttention`, and `.thl-*` CSS deleted. Status kanban is the only rendering. Unread dots + Mark-all-read stay, on the cards. `verify:hypotheses` rewritten to board-only contract (13 assertions green). Stale stored prefs dropped by `mergePrefs` rebuilding a fresh object (pinned by verify assertion "a stale list pref cannot resurrect the list")
