...
- **Skills enumerate all capabilities** — never claim "we don't have X" without checking references.
- **In-app terminal: comfort over sharpness** — JetBrains Mono 14.5px / 1.65, native AA.
- **Lab is "Insights" in the UI** (the `lab` CLI is an implementation detail).
- **Turkish must survive every transform**: he writes names, tags and directives in Turkish. Slugs fold to ASCII (`foldToAscii`), display uses the frontmatter name verbatim, search is case-insensitive. Mangling Turkish is a bug, not cosmetics.
