...
**Or the same agent as a native chat** _(beta, opt-in)_. Settings → Agents → Agent screen switches the surface from the raw terminal to a **Chat view**: the same `claude` binary and the same permission model, rendered as app UI instead of a TUI. Streaming answers, tool calls as collapsible cards with real diffs, Claude's questions and permission prompts as clickable cards, dispatched sub-agents grouped into one live card you can drill into, `/` for the skill picker, and hover actions to copy, quote, rewind or retry any message. Model and effort switch mid-conversation, and a conversation started in either surface resumes in the other.

Here the agent also knows it isn't in a terminal, so it can hand you the thing itself instead of a path to go find: pictures and clips play in the conversation, an Excalidraw board it just drew appears **as the board** on a live pan/zoom canvas, file paths it mentions are clickable, and it can put a row of buttons under an answer — open that task, open that board, send the follow-up it just proposed. You stay in the chat.
