...
**Or the same agent as a native chat** — now the standard screen. **Chat view** is the same `claude` binary and the same permission model, rendered as app UI instead of a TUI (the raw terminal is still there as **Terminal (legacy)** in Settings → Agents). Streaming answers, tool calls as collapsible cards with real diffs, Claude's questions and permission prompts as clickable cards, dispatched sub-agents grouped into one live card you can drill into, `/` for the skill picker anywhere in a prompt, and hover actions to copy, quote, rewind or retry any message. ⏎ while a turn is running queues your next message as an editable row instead of doing nothing, ↑/↓ walk the prompts you already sent, ⌃C stops the turn, and background shells the agent starts get their own tray you can read and stop without spending a turn. Model and effort switch mid-conversation, and a conversation started in either surface resumes in the other.
