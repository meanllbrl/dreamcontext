...
- **Every automation is a channel you can talk back in.** Each fire becomes a thread, and the run itself decides what to say — a finding, a number that moved, a document, an image, a live board — so an unremarkable run stays quiet instead of narrating itself. The Agents page reads as one message per run across every agent, and you never open a session to follow along. **Reply in a thread and the run's own session resumes** on the machine that ran it, answering as a post underneath; `@name` in the channel does the same, or starts a single run for an agent that only works when called. A run that needs a decision asks with real buttons rather than a sentence you have to answer in prose.
