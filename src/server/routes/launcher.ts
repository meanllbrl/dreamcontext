...
/**
 * In-memory state of a background Claude run, keyed by run id. Started by the
 * Sleep consolidation route, which returns the id; the caller polls for state and
 * output. Purely ephemeral — lost on server restart.
 */
...
//
// Same rationale as agent-ui.json: the app's per-launch loopback port resets
...
//
// Same rationale as agent-ui.json: the app picks a FRESH loopback
